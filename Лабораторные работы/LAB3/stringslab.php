...
*/
    ...
    if (!preg_match("/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/",$password)) echo "Пароль простой";
        else echo "Пароль сложный";

    echo "<br/>".mb_strtoupper(mb_substr($name, 0, 1)).mb_substr($name, 1)."<br/>";

    echo $email." - ";
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) echo "корректная почта";
    else echo "не корректная почта";
